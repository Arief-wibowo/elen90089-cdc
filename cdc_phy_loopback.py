...
class cdc_phy_loopback(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "CDC PHY Loopback", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("CDC PHY Loopback")
        ...
        self.settings = Qt.QSettings("GNU Radio", "cdc_phy_loopback")
        ...
        self.phase_rot = phase_rot = np.pi/4
        self.nbytes = nbytes = 32
        self.excess_bw = excess_bw = 0.35
        self.amp_gain = amp_gain = 0.0

        ##################################################
        # Blocks
        ##################################################

        self._phase_rot_range = Range(-np.pi, np.pi, 2*np.pi/200, np.pi/4, 200)
        self._phase_rot_win = RangeWidget(self._phase_rot_range, self.set_phase_rot, "Phase Rot", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_layout.addWidget(self._phase_rot_win)
        self._amp_gain_range = Range(-3, 3, 0.1, 0.0, 200)
        self._amp_gain_win = RangeWidget(self._amp_gain_range, self.set_amp_gain, "Amp Gain", "counter_slider", float, QtCore.Qt.Horizontal)
        ...
        self.qtgui_const_sink_x_0.set_trigger_mode(qtgui.TRIG_MODE_TAG, qtgui.TRIG_SLOPE_POS, 0.0, 0, "corr_est")
        ...
        self.blocks_message_strobe_1 = blocks.message_strobe(pmt.cons(pmt.PMT_NIL, pmt.make_u8vector(nbytes, 0x00)), 500)
        ...
        # Connections
        ##################################################
        self.msg_connect((self.blocks_message_strobe_1, 'strobe'), (self.phy_tx_0, 'sdu'))
        ...
        self.connect((self.cdc_insert_burst_c_0, 0), (self.blocks_throttle2_0, 0))
        self.connect((self.channels_channel_model_0, 0), (self.phy_rx_0, 0))
        self.connect((self.phy_rx_0, 2), (self.blocks_uchar_to_float_0, 0))
        self.connect((self.phy_rx_0, 0), (self.qtgui_const_sink_x_0, 0))
        self.connect((self.phy_rx_0, 1), (self.qtgui_const_sink_x_0_0, 0))
        self.connect((self.phy_rx_0, 3), (self.qtgui_time_sink_x_0, 0))
        self.connect((self.phy_rx_0, 4), (self.qtgui_time_sink_x_1, 0))
    ...
    def set_phase_rot(self, phase_rot):
        ...
        self.channels_channel_model_0.set_taps([np.exp(1j*self.phase_rot)*10**(self.amp_gain)])
    ...
    def set_nbytes(self, nbytes):
        self.nbytes = nbytes
        self.blocks_message_strobe_1.set_msg(pmt.cons(pmt.PMT_NIL, pmt.make_u8vector(self.nbytes, 0x00)))
...
def main(top_block_cls=cdc_phy_loopback, options=None):
